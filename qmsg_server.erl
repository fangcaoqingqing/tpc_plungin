...
sendmsg(Uid, Msg) when is_integer(Uid), is_list(Msg) ->
    ...
    io:format("ReqBin server sendmsg:  ~w~n",[ReqBin]),
    ?LOG("ReqBin server sendmsg:  ~w~n", [ReqBin], debug),
